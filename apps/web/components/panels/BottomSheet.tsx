  ...
  return (
    ...
    >
      <div className={`${
        ...
        >
          <div className={`w-12 h-1.5 ${theme === 'light' ? 'bg-slate-300 group-hover:bg-slate-400' : 'bg-slate-600 group-hover:bg-slate-400'} rounded-full mb-1 transition-colors`} />
          <div className={`flex items-center gap-1 text-[11px] font-semibold ${theme === 'light' ? 'text-slate-600' : 'text-textSecondary'}`}>
        ...
        <div className="p-3 pt-0 space-y-3 overflow-y-auto max-h-[72vh] pb-6">
          ...
          {isExpanded && (
            ...
            >
              ...
              {/* Station Halts List */}
              <div className={`${
                theme === 'light' ? 'bg-white border-slate-300 shadow-lg' : 'bg-bgCard border-slate-700/80 shadow-xl'
                ...
                <div className="space-y-2 text-xs">
                  <div className="flex items-center justify-between p-2 rounded-lg bg-emerald-500/10 border border-emerald-500/20 text-emerald-600 dark:text-emerald-300">
                    ...
                    <span className="font-mono">{train.nextStation.distanceKm} km away • ETA {train.nextStation.etaMinutes}m</span>
                  </div>

                  <div className={`flex items-center justify-between p-2 rounded-lg ${
                    theme === 'light' ? 'bg-slate-50 border-slate-200 text-slate-700' : 'bg-slate-900 border-slate-800 text-slate-400'
                  } border`}>
